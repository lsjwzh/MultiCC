...
The eleven documents the README links to directly.
...
| [Installation & service management](installation.md) | Install-script flags (`--dir`, `--version`, `--from`, `--port`, `--token`, `--no-service`), updating, force-push recovery, the `./multicc` manager, macOS `launchd` / Linux `systemd` units, release APK distribution, and iOS builds. |
...
| [Standalone package](standalone.md) | The distribution form everything else wraps: layout, the `multicc` command, verified in-place updates, data locations, and why it runs on macOS 11+/Intel with no Node, no compiler and no git. |
...
| [Task shells and context branches experiment](task-session-shell-experiment.md) | Current proposal: many-to-many shell/task links, shared task queues, canonical messages, context freeze/resume, side effects, and experimental acceptance gates. Not yet implemented. |
| [Earlier task context isolation proposal](task-context-isolation-design.md) | Earlier context/archive analysis, superseded where noted by the task-shell experiment. Not implemented. |

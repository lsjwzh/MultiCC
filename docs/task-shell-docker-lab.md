...
本环境用于任务会话壳实验的 Linux 回归和手动验收。普通 Web 会话、真实模型额度、生产数据库和宿主项目工作区均不参与测试。分叉生成独立任务，不汇合；后续新任务通过已完成上下文快照引用之前的工作。
...
| 单元与 HTTP | 任务关系、忙时分叉、回执幂等、跨项目限制、快照、容量、控制消息 |
...
1. 在壳 A 新建任务，单独一行发送 `LAB_WAIT 30`，任务保持运行约 30 秒（上限 120 秒）。
2. 在壳 B 的关联菜单选择 A 的任务，发送普通新工作。应创建另一个任务，A 保持原有执行。
3. 切回 A，点击取消。只应取消 A；B 已完成的输出保持可见。
4. 在 B 选择“新任务”，勾选已完成任务为上下文来源后发送。检查来源快照；运行中与取消的输出不应成为已完成上下文。
5. 单独一行发送 `LAB_FAIL`，验证执行失败展示；新建另一个普通任务应仍可完成。
6. 用 `lab:down` 停止后重新 `lab:up`，两个壳的 ID、历史、任务关系应保留，重复启动不增加预置会话。

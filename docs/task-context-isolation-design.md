...
> 后续讨论形成了新的[多会话壳与任务上下文分支实验设计](task-session-shell-experiment.md)。新实验以该文为准：增加壳与任务多对多、统一任务调度和跨壳问题结算；将本文「完成后追问必建新 epoch」改为「完成后先冻结，验证后可恢复原分支」。本文保留为前版分析记录，其阶段与文件数量不与新方案累加。

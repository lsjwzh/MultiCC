...
The summary service (Aux) can select **Codex 官方** in its OpenAI provider list and use the models listed for that provider. It follows the globally selected official account through the local relay; no API key or borrowed provider is required. Responses requests use message-array input and consume the completed streaming response. A failed or interrupted stream is reported as an error rather than saved as a partial summary.

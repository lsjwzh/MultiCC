...
日期：2026-09-06。状态：已实现的默认关闭实验；确定性、隔离宿主及真实 Chrome 验收见 [发布测试记录](task-shell-release-tests.md)。真实模型的效果、成本和长期稳定性仍需启用后的试用数据。

本版替代之前设计中的「多个壳向忙任务默认排队」「完成后必建 epoch」「分叉后汇合」设想。实现遵循用户最后确认的规则：空闲继续；占用时独立工作分叉；各分支不汇合；后续新任务按需引用完成上下文。

## 1. 使用与路由

启用 `MULTICC_TASK_SHELLS=1` 后，由用户手动重启服务。普通 Claude / Codex Web chat 的「清理显示」菜单出现「任务会话 · 实验」入口，也可打开 `/task-shell.html?session=<普通会话 ID>`。打开页面会创建与来源会话关联的壳；原聊天保持原样，旧聊天历史不会隐式导入。

每个壳可以关联同项目的多个实验任务，多个壳可以关联同一任务。当前任务选择保存在浏览器当前标签页，后台输出不会改变选择。
...
| 原生 JSONL | CLI 执行缓存，保持原有管理；本实验不并发改写或复制它 |
...
实验执行会话的 `autoCommit=false`，关闭宿主的自动提交 / 合并；任务身份合并 API 拒绝实验任务。用户明确发起的仓库操作仍遵守原有授权与 Git 规则。

## 4. 改动范围

| 文件 / 模块 | 变动与影响 |
| --- | --- |
| `src/task-shell/{store,context,runtime,host,routes}.js` | 新增实验状态、路由、快照、宿主适配和 HTTP API |
| `src/paths.js`、运行时写入治理清单 | 新增数据根内 SQLite 路径，独立于原数据库 |
| `server.js` | 小范围组合接线、内部创建参数允许关闭 autoCommit、关闭时释放 SQLite |
| `src/task-context-host.js`、`src/chat/turn-engine.js` | 入队 / 执行前身份门禁、冷启动 seed、旧 WS 输入 / 停止 / 清理拒绝 |
| `src/chat/turn-request.js`、`src/task-board.js` | 声明可信 `task-shell` 来源，避免实际执行被拒为 invalid_task |
| `src/session-work-host.js`、`src/session-work-scheduler.js`、`src/orchestration-runtime.js` | 仅对携带实验凭据的投递启用严格回合 / 问题校验和跨回合重试；普通消息维持原语义 |
...
| `public/chat-context-controls.js`、`public/chat.js` | 开关开启时，普通聊天显示入口 |
...
新增 API 位于 `/api/task-shells`，经过现有主应用认证，不为公开分享授权开放。任务 / 引用强制同项目且先关联。API 不接受 CLI 状态、任意路径、原生会话 ID 或跨项目拷贝参数。旧入口向实验执行会话投普通输入会返回 `task_shell_route_required`；原系统回调 / 重试保持原执行会话身份。

## 5. 副作用与边界

1. **额外资源与费用。** 分叉增加工作区和模型上下文成本。本版每项目最多 4 个占用实验任务、200 个任务；超并发限制时返回可重试容量错误，并保留原目标。等待问题和未知状态也保守计入占用；回答 / 取消不被容量门槛阻断。并发门槛仅约束实验用户工作，不替代整个宿主的资源管理。
2. **上下文滞后与截断。** 来源仍在进行的工作不进入快照；引用过长会按完整回合省略。省略是可见的，正式原历史仍保存。实验页目前显示最近 200 条消息，不是归档浏览器。
...
6. **显示有延迟。** 实验页轮询正式历史，最长约两秒刷新；不是新的 token 级流式渲染器。旧完整聊天渲染与 App 主流程未迁移。
7. **身份和上下文迁移有限。** 只支持本机、同项目、普通 Claude / Codex Web chat 创建的新实验任务；旧任务自动迁移、其他 CLI 资格验证、App 专属入口、语义自动归集和自动依赖调度属于后续阶段。
8. **持久数据会增长。** 本版无自动任务 / 快照 GC，不删除原生历史。移除壳只拆关联，任务与其他壳继续存在；应纳入已有数据目录备份。SQLite 运行时备份应使用 SQLite backup 或一致性停机备份，不能只复制活跃主文件而忽略 WAL。

## 6. 上线与回退

默认 `MULTICC_TASK_SHELLS` 未设置，旧聊天流程不变。合入后用户手动开启环境开关并重启，再从普通聊天菜单进入。先在单项目试用，观察重复执行、控制消息误投、快照覆盖、资源开销和实际上下文质量。

回退时关闭开关并由用户手动重启。已创建壳可通过原壳 URL 读取历史；新投递 / 新关联关闭，已有执行按原持久调度恢复。SQLite、隐藏聊天历史和工作区保留，不做自动删除。功能开关回退不等同于任意旧二进制降级；版本降级前应让实验任务结束并备份数据。

本次没有设置生产开关、没有重启用户服务，也没有发布版本标签或安装包。发布资格是「默认关闭的 Web 实验」，不是全量替换现有任务引擎。

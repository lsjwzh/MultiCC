...
更新：2026-09-07。状态：已固化为普通聊天的消息入口；确定性、隔离宿主及真实 Chrome 验收见 [发布测试记录](task-shell-release-tests.md)。真实模型的效果、成本和长期稳定性仍需启用后的试用数据。

任务壳持有服务端权威 `currentTaskId`。普通消息始终顺着最近任务投递；任务占用时进入同一任务队列，不再自动分叉。只有用户点击「新任务」才会在执行前创建独立任务和工作区。每轮默认不注入其他任务历史，模型只有在当前上下文不足时才能调用只读 MCP `get_task_context` 回补带来源及执行状态的历史。
...
2. **上下文滞后与截断。** 默认回补排除当前仍运行的回合，主动分页可查并看到 inProgress 标记。交接保留失败/部分输出状态，长证据明确标为节选并提供原文游标。任务详情显示最近 200 条归属消息，完整聊天壳支持分页。

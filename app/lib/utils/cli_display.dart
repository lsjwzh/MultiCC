...
// colour / providerless 四列）。Web 侧镜像在 public/provider-catalog.js，这里是 App
// 侧镜像；tests/test-cli-display-parity.js 读这三份，任何一列漂移就红。
//
...
/// 一个 CLI 的展示事实：名字、品牌色、折叠徽标上的字母、是否自持账号。
class CliDisplay {
  const CliDisplay(this.name, this.color, this.mark, {this.providerless = false});
...
const Map<String, CliDisplay> kCliDisplays = <String, CliDisplay>{
  ...
  'codex': CliDisplay('Codex', AppColors.codex, 'X'),
  'codex-exp': CliDisplay('Codex Exp', AppColors.codex, 'E'),

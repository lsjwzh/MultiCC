...
///   勾选           → 禁止派发，活留在当前会话做完
///
/// 两条后缀和 web 端 `public/chat-dispatch-hint.js` 逐字一致：route_task 是
/// Commander 唯一的派发通道，用它自己的词汇下指令，模型才知道怎么遵守。
/// 非 commander 会话一律 fail closed —— [decorateDispatchHint] 原样返回。
library;

const String kDispatchHintSuffixSpread =
    '\n\n[派发要求] 请尽可能把本次任务拆分，并通过 route_task 派发给其它会话并行完成；'
    '只有确实无法派发的部分才留在当前会话自己做。';

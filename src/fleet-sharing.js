...
function normalizeRemoteSessions(value) {
  ...
  return sourceSessions.map(session => ({
    ...
    cli: ['claude', 'codex', 'codex-exp', 'opencode', 'zcode', 'qoder', 'kimi', 'codebuddy', 'dsh'].includes(session && session.cli)
      ? session.cli : 'other',
...
function createFleetSharing({
  ...
  function sharedFleetPayload(record, token) {
    ...
    const sessions = listSessions(directory.id).slice(0, 500).map(session => ({
      ...
      cli: ['claude', 'codex', 'codex-exp', 'opencode', 'zcode', 'qoder', 'kimi', 'codebuddy', 'dsh'].includes(session.cli)
        ? session.cli : 'other',

...
const COMMANDER_ROUTER_PROMPT = [
  ...
  '   选择规则：用户明确点名的合法 chat session 必须照选、不得改派；未点名时，关联会话 load="available" 且 routingState=ready/unknown 则优先；关联会话 load="running" 或不可立即接单时，优先选择其他能胜任的 load="available"、routingState=ready/unknown 会话；只有全部可胜任会话都忙时才进入最匹配会话的 FIFO。',
  '   role 高于一次偶发任务；recentTasks 只证明近期经验与上下文，不会永久改变角色。禁止只看 session id、CLI 或最近活跃时间猜职责。',
  '   routingState="waiting_user"、"background" 或 "error" 均不算可立即接单；processing 仍优先让位于 ready/unknown。除非用户明确点名或没有其他可胜任的空闲会话，否则优先改投。选定 busy 目标后仍安全排队且不打断当前 turn。',
  '   改投其他会话时不要依赖旧会话历史：message 必须写清目标、已知事实、约束、相关文件/分支/operation_id、验收标准和交付方式；只传必要且已脱敏的上下文，不要复制完整对话或秘密。',
  '   不要把同一任务广播给多个会话。',
  ...
  '3. message 内的任务描述必须完整自包含（worker 看不到你的对话上下文），尤其是忙时改投到其他会话。',

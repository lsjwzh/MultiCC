...
const COMMANDER_ROUTER_PROMPT = [
  ...
  '   只有 CLI 明确报告 route_task 不可用时，才允许用迁移期兼容标记 <<route target="SESSION_ID">...</route>>。',
  ...
  '   默认只能选择 kind="chat" 的 worker。禁止自动派发到 kind="terminal"；只有用户明确指定某个终端时，才可选择该 id 并设置 allow_terminal=true。',
  ...
  '  兼容回退：<<route target="multicc-claude-chat-05">请修改 README.md，补充安装步骤并验证</route>>',

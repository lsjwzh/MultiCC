...
test('lab service owns HTTP/WS listener and forwards stop only to its Compose child', async t => {
  ...
  fs.writeFileSync(fake, `#!${process.execPath}\nconst fs=require('fs');fs.writeFileSync(${JSON.stringify(marker)},'started');setInterval(()=>{},1000);process.on('SIGTERM',()=>{fs.writeFileSync(${JSON.stringify(marker)},'stopped');process.exit(0)});`, { mode: 0o755 });
  const backend = http.createServer(async (req, res) => {
    const chunks = []; for await (const chunk of req) chunks.push(chunk);
    res.end(req.url + ':' + Buffer.concat(chunks));
  });
  ...
  const response = await fetch(`http://127.0.0.1:${port}/echo?x=1`, { method: 'POST', body: 'body' });
  assert.equal(await response.text(), '/echo?x=1:body');
  const ws = new WebSocket(`ws://127.0.0.1:${port}/ws`); await once(ws, 'open');
  const received = once(ws, 'message'); ws.send('payload'); assert.equal(String((await received)[0]), 'payload');

...
// ============================================================================
// 记忆图谱 · Memory Graph  (无第三方依赖：力导向布局 + SVG 渲染 + 平移/缩放/拖拽)
// 数据来源：GET /api/memory/graph  →  { nodes:[{id,title,summary,type,scope,…}], edges:[{source,target,type,strength}], meta }
// ============================================================================
(function () {
  ...
  window.loadMemoryGraph = async function (dirId, forceRefetch) {
    ...
    if (!_memRaw || forceRefetch) {
      ...
      try {
        const r = await fetch('/api/memory/graph' + tokenQS('?'));
  ...
  window.memNodeModalOpen = function (id) {
    ...
    el('mem-node-title').textContent = nd.title || nd.slug;
    const scopeTxt = nd.scope === 'shared' ? '公共记忆' : nd.scope === 'session' ? ('会话私有 · ' + (nd.sessionId || '')) : nd.scope;
    el('mem-node-slug').textContent = nd.file + '   ·   ' + scopeTxt;
    const tags = el('mem-node-tags');
    tags.innerHTML = '';
    ...
    // 邻居（出/入边）
    const linksBox = el('mem-node-links'); linksBox.innerHTML = '';
    const out = [], inc = [];
    for (const e of G.edges) {
    ...
    el('mem-node-modal').classList.add('open');
  };

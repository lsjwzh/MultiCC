...
description: Use the @cloudflare/computer package to give a Durable Object a durable SQLite-backed filesystem (workspace.fs) and pluggable command/code execution backends (container, worker-shell, worker-javascript), including AI SDK tools, git, and stub disposal rules. Load when working with @cloudflare/computer, Workspace, withWorkspace/getWorkspace, workspace.fs, or runtime.exec.
---

# Using `@cloudflare/computer`

`@cloudflare/computer` gives a Cloudflare Durable Object a persistent,
SQLite-backed virtual filesystem plus pluggable command and code
execution. It is built for agents that need a small, portable working
directory and the tools to read, write, and run things in it.

> **PREVIEW ONLY.** The package is provided as a preview for feedback
> only. APIs are unstable and the design is subject to change. Suitable
> for experiments and prototypes; NOT suitable for production use.

Upstream: <https://github.com/cloudflare/computer> (MIT license — see
the `LICENSE` file next to this skill).

## What you get

- **A filesystem in your Durable Object.** `workspace.fs` looks like
  `node:fs/promises` — `readFile`, `writeFile`, `mkdir`, `readdir`,
  `rm`, `grep` — durable across DO restarts, backed by the DO's own
  SQLite storage.
...
The Workspace can also run with no execution backend at all — just the
filesystem.

### Limits

- ~10 GB per workspace (shares storage with the DO).
- The container-side filesystem is held in memory. Aim for agent-scale
  workspaces, not full monorepos.
- Container access goes through FUSE, so heavy I/O (large
  `node_modules` installs, big tarball extractions) is slower than a
  native disk.
...
Pre-fill part of the tree from an R2 bucket. Files under the mount
point are read-only; writes reject with `EROFS`.
...
The handle is also a `ReadableStream` of live events, so you can
forward output as it happens — for example, as Server-Sent Events:
...
- **Container** cold-starts more slowly but gives you a real Linux
  environment. The container owns its own SQLite-backed VFS and the
  package syncs the two stores across a capnweb WebSocket.
- **Worker shell** is fast and needs no container. Every filesystem
  ...
  second store and no sync round trip.
...
command should run, so write them in plain language.
...
There's also an argv entry point (`workspace.git.cli({ argv })`), and
when git is configured the worker-shell backend exposes a built-in
`git` command.
...
- **Assets** (`@cloudflare/computer/assets`): `createAssets(...).share`
  ...
  `assets publish <path> [<expiry>]` command.
...
stubs or on busy `exec` workloads. To hunt leaks, set
`CAPNWEB_TRACK_STUBS=1` and read `stubSnapshot()` from
`@cloudflare/computer-rpc/debug`, or hit `GET /__computerd/stubs` on a
computerd instance.
...
- Upstream repo and examples: <https://github.com/cloudflare/computer>
  (`examples/worker-shell`, `examples/worker-javascript`,
  `examples/container`, `examples/think`, `examples/tutorial`).
- Design spec under the repo's `docs/` — forward-looking; read it for
  intent, not as a description of the code today.
- For broader Cloudflare platform guidance, load the companion
  `cloudflare-platform-skills` skill.

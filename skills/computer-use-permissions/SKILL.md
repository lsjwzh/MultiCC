...
description: 为 computer-use 第三方技能（cliclick / _cu_scroll / node）授予 macOS 辅助功能权限。运行 setup.sh 会同时弹出「系统设置 → 隐私与安全性 → 辅助功能」页面和 Finder（高亮待授权的二进制），方便用户拖拽或 ⌘⇧G 添加。含 Touch ID 之后的自动导航与验证流程。
---

# computer-use 权限设置技能

给本机 computer-use 工具链（第三方技能 oil-oil/computer-use-skill）授予 macOS 辅助功能（Accessibility）权限。
...
- 授权按**二进制路径**绑定，所以工具必须放在稳定路径（本技能统一用 `/opt/homebrew/bin/`）。computer-use 技能的 init.sh 每次会往 `/var/folders/.../T/computer-use.*` 编译临时副本——那个永远没权限，**不要授权临时副本**，让用户始终添加 `/opt/homebrew/bin/` 下的稳定版。
...
脚本会：
1. 确保 `/opt/homebrew/bin/` 下有 `cliclick`（缺失时提示 brew install）和 `_cu_scroll`（缺失时从本技能 `scripts/scroll.swift` 现场编译，或从 computer-use 技能的临时目录复制）；
2. `open` 辅助功能设置锚点页面；
...
```bash
# 检测文件选择框弹出（node 已授权时 System Events 可用）
...
# ⌘⇧G → 粘贴完整路径（含文件名）→ 回车
osascript -e 'set the clipboard to "/opt/homebrew/bin/_cu_scroll"'
/opt/homebrew/bin/cliclick kd:cmd kd:shift t:G ku:shift ku:cmd; sleep 0.8
/opt/homebrew/bin/cliclick kd:cmd t:v ku:cmd; sleep 0.5
/opt/homebrew/bin/cliclick kp:return; sleep 1
...
也可以让用户手动：把 Finder 里高亮的文件**拖进**辅助功能列表，或在文件选择框里按 `⌘⇧G` 粘贴完整路径（必须含文件名，目录浏览看不到 /opt/homebrew）。

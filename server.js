...
const ttsService = require('./src/voice/tts-service');
...
// Host-authored chat prompt add-ons (codex ask-tool steering, stay-alive rules,
// the multicc web-chat hint block) live in src/chat/host-prompts.js.
const {
...
function handleProxyOutcome(event) {
  const outcome = providerAttemptRuntime.observeProxyOutcome(event);
...
const cliSwitchRuntime = createCliSwitchRuntime({
  ...
  codexDefaultReasoningLevel, synchronizeCodexSessionRoute: route => providers.synchronizeCodexSessionRoute(route),
